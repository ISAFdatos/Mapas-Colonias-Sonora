...
capa_mza <- st_read("SHAPES/INEGI/26mcol.shp", quiet = TRUE,options = "ENCODING=windows-1252") 
...
capa_col <- st_read("SHAPES/INE/COLONIA_INEGI.shp", quiet = TRUE,options = "ENCODING=windows-1252") 

capa_col <- st_transform(capa_col, crs = CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")) %>% 
  clean_names() %>% 
  st_make_valid() 

colonia_manzana <- capa_mza %>% as.data.frame() %>% select(cvegeo, cve_col, nom_col)
...
soncol <- sonmza %>% 
  group_by(censo, nom_loc, cve_col, nom_col) %>% 
...
plot_tinaco_pretty <- function(y = y) {
